...
const SimplePayrollList: React.FC = () => {
  ...
  // Listen for SSE events and refresh data when payroll is signed
  useEffect(() => {
    const payrollSignedEvents = events.filter(
      (event) => event.type === "payroll_signed"
    );
    if (payrollSignedEvents.length > 0) {
      // Refresh data when someone signs payroll
      console.log("📡 SSE: Payroll signed event detected, refreshing data...");
      fetchPayrollData(currentPage, searchKeyword);
    }
  }, [events, currentPage, searchKeyword]);
...
export default SimplePayrollList;
